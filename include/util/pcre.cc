...
int pcre_exec(const pcre*, const pcre_extra*, const char*, int, int, int, int*,
              int) {
  return 0;
}
...
bool PCRE::Replace(std::string* str, const PCRE& pattern,
  ...
  std::string s;
  if (!pattern.Rewrite(&s, rewrite, *str, vec, matches)) return false;
...
int PCRE::GlobalReplace(std::string* str, const PCRE& pattern,
  ...
  while (start <= str->size()) {
    ...
    if (last_match_was_empty_string) {
      ...
      if (matches <= 0) {
        if (start < str->size()) out.push_back((*str)[start]);
        start++;
  ...
  if (start < str->size()) out.append(*str, start, str->size() - start);
  using std::swap;
  swap(out, *str);
  return count;
}
...
int PCRE::TryMatch(const StringPiece& text, size_t startpos, Anchor anchor,
  ...
  int options = 0;
  if (anchor != UNANCHORED) options |= PCRE_ANCHORED;
  if (!empty_ok) options |= PCRE_NOTEMPTY;
