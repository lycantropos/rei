...
PrefilterTree::~PrefilterTree() {
  for (size_t i = 0; i < prefilter_vec_.size(); i++) delete prefilter_vec_[i];

  for (size_t i = 0; i < entries_.size(); i++) delete entries_[i].parents;
}
...
void PrefilterTree::Compile(std::vector<std::string>* atom_vec) {
  ...
  if (prefilter_vec_.empty()) return;
  ...
  if (ExtraDebug) PrintDebugInfo(&nodes);
}
...
bool PrefilterTree::KeepNode(Prefilter* node) const {
  ...
  switch (node->op()) {
    ...
    case Prefilter::OR:
      for (size_t i = 0; i < node->subs()->size(); i++)
        if (!KeepNode((*node->subs())[i])) return false;
      return true;
  }
}
...
void PrefilterTree::AssignUniqueIds(NodeMap* nodes,
  ...
  for (size_t i = 0; i < prefilter_vec_.size(); i++) {
    ...
    if (f == NULL) unfiltered_.push_back(static_cast<int>(i));
  ...
  for (size_t i = 0; i < v.size(); i++) {
    ...
    if (f->op() == Prefilter::AND || f->op() == Prefilter::OR) {
      ...
      for (size_t j = 0; j < subs.size(); j++) v.push_back(subs[j]);
    }
  }
  ...
  for (int i = static_cast<int>(v.size()) - 1; i >= 0; i--) {
    ...
    if (CanonicalNode(nodes, prefilter) != prefilter) continue;
  ...
  for (size_t i = 0; i < prefilter_vec_.size(); i++) {
    if (prefilter_vec_[i] == NULL) continue;
...
void PrefilterTree::PropagateMatch(const std::vector<int>& atom_ids,
  ...
  for (size_t i = 0; i < atom_ids.size(); i++) work.set(atom_ids[i], 1);
  for (IntMap::iterator it = work.begin(); it != work.end(); ++it) {
    ...
    for (StdIntMap::iterator it = entry.parents->begin();
      ...
      if (parent.propagate_up_at_count > 1) {
        ...
        if (c < parent.propagate_up_at_count) continue;
      }
      // Trigger the parent.
      work.set(j, 1);
    }
...
void PrefilterTree::PrintDebugInfo(NodeMap* nodes) {
  ...
  LOG(ERROR) << "Map:";
  for (NodeMap::const_iterator iter = nodes->begin(); iter != nodes->end();
       ++iter)
