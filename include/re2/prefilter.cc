...
Prefilter::Prefilter(Op op) {
  op_ = op;
  subs_ = NULL;
  if (op_ == AND || op_ == OR) subs_ = new std::vector<Prefilter*>;
}
...
std::string Prefilter::Info::ToString() {
  ...
  if (match_) return match_->DebugString();
...
static void CopyIn(const std::set<std::string>& src,
  ...
  for (ConstSSIter i = src.begin(); i != src.end(); ++i) dst->insert(*i);
}
...
static void CrossProduct(const std::set<std::string>& a,
  ...
  for (ConstSSIter i = a.begin(); i != a.end(); ++i)
    for (ConstSSIter j = b.begin(); j != b.end(); ++j) dst->insert(*i + *j);
}
...
Prefilter::Info* Prefilter::Info::CClass(CharClass* cc, bool latin1) {
  ...
  if (ExtraDebug) LOG(ERROR) << " = " << a->ToString();
...
Prefilter::Info* Prefilter::BuildInfo(Regexp* re) {
  if (ExtraDebug) LOG(ERROR) << "BuildPrefilter::Info: " << re->ToString();
...
Prefilter::Info* Prefilter::Info::Walker::PostVisit(
  ...
  switch (re->op()) {
    ...
    case kRegexpAlternate:
      info = child_args[0];
      for (int i = 1; i < nchild_args; i++) info = Alt(info, child_args[i]);
      break;
