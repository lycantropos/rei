...
// (https://gcc.gnu.org/bugzilla/show_bug.cgi?id=70932)
  ...
  struct StateHash {
    size_t operator()(const State* a) const {
      DCHECK(a != NULL);
      HashMix mix(a->flag_);
      for (int i = 0; i < a->ninst_; i++) mix.Mix(a->inst_[i]);
      mix.Mix(0);
      return mix.get();
    }
  };
  ...
  struct StateEqual {
    bool operator()(const State* a, const State* b) const {
      ...
      DCHECK(b != NULL);
      if (a == b) return true;
      if (a->flag_ != b->flag_) return false;
      if (a->ninst_ != b->ninst_) return false;
      for (int i = 0; i < a->ninst_; i++)
        if (a->inst_[i] != b->inst_[i]) return false;
      return true;
    }
  };
 ...
 private:
  ...
  int ByteMap(int c) {
    if (c == kByteEndText) return prog_->bytemap_range();
...
DFA::DFA(const Prog* prog, Prog::MatchKind kind, int64_t max_mem)
  ...
  int nmark = 0;
  if (kind_ == Prog::kLongestMatch) nmark = prog_->size();
...
std::string DFA::DumpState(State* state) {
  if (state == NULL) return "_";
  if (state == DeadState) return "X";
  if (state == FullMatchState) return "*";
  std::string s;
  const char* sep = "";
...
DFA::State* DFA::WorkqToCachedState(Workq* q, Workq* mq, uint32_t flag) {
  ...
  for (Workq::iterator it = q->begin(); it != q->end(); ++it) {
    int id = *it;
    if (sawmatch && (kind_ == Prog::kFirstMatch || q->is_mark(id))) break;
    if (q->is_mark(id)) {
    ...
    switch (ip->opcode()) {
      case kInstAltMatch:
        ...
        if (kind_ != Prog::kManyMatch &&
          ...
          if (ExtraDebug) fprintf(stderr, " -> FullMatchState\n");
      ...
      default:
        ...
        if (prog_->inst(id - 1)->last()) inst[n++] = *it;
        if (ip->opcode() == kInstEmptyWidth) needflags |= ip->empty();
        if (ip->opcode() == kInstMatch && !prog_->anchor_end()) sawmatch = true;
        break;
    }
  }
  DCHECK_LE(n, q->size());
  if (n > 0 && inst[n - 1] == Mark) n--;
  ...
  // of distinct states.)
  if (needflags == 0) flag &= kFlagMatch;
  ...
  if (n == 0 && flag == 0) {
    if (ExtraDebug) fprintf(stderr, " -> DeadState\n");
    return DeadState;
  }
  ...
  if (kind_ == Prog::kLongestMatch) {
    ...
    while (ip < ep) {
      int* markp = ip;
      while (markp < ep && *markp != Mark) markp++;
      std::sort(ip, markp);
      if (markp < ep) markp++;
      ip = markp;
    }
...
DFA::State* DFA::CachedState(int* inst, int ninst, uint32_t flag) {
  ...
  if (it != state_cache_.end()) {
    if (ExtraDebug) fprintf(stderr, " -cached-> %s\n", DumpState(*it).c_str());
    return *it;
  }
  ...
  s->ninst_ = ninst;
  s->flag_ = flag;
  if (ExtraDebug) fprintf(stderr, " -> %s\n", DumpState(s).c_str());
...
DFA::State* DFA::RunStateOnByte(State* state, int c) {
  ...
  if (ismatch) flag |= kFlagMatch;
  if (isword) flag |= kFlagLastWord;
...
DFA::StateSaver::~StateSaver() {
  if (!is_special_) delete[] inst_;
}

DFA::State* DFA::StateSaver::Restore() {
  if (is_special_) return special_;
  MutexLock l(&dfa_->mutex_);
  State* s = dfa_->CachedState(inst_, ninst_, flag_);
  if (s == NULL) LOG(DFATAL) << "StateSaver failed to restore state.";
  return s;
}
...
inline bool DFA::InlinedSearchLoop(SearchParams* params) {
  ...
  State* s = start;
  if (ExtraDebug) fprintf(stderr, "@stx: %s\n", DumpState(s).c_str());

  if (s->IsMatch()) {
    matched = true;
    lastmatch = p;
    if (ExtraDebug) fprintf(stderr, "match @stx! [%s]\n", DumpState(s).c_str());
    if (params->matches != NULL && kind_ == Prog::kManyMatch) {
      for (int i = s->ninst_ - 1; i >= 0; i--) {
        int id = s->inst_[i];
        if (id == MatchSep) break;
  ...
  while (p != ep) {
    if (ExtraDebug) fprintf(stderr, "@%td: %s\n", p - bp, DumpState(s).c_str());
  ...
  if (ExtraDebug) fprintf(stderr, "@etx: %s\n", DumpState(s).c_str());
  ...
  if (s->IsMatch()) {
    matched = true;
    lastmatch = p;
    if (ExtraDebug) fprintf(stderr, "match @etx! [%s]\n", DumpState(s).c_str());
    if (params->matches != NULL && kind_ == Prog::kManyMatch) {
      for (int i = s->ninst_ - 1; i >= 0; i--) {
        int id = s->inst_[i];
        if (id == MatchSep) break;
...
bool DFA::AnalyzeSearch(SearchParams* params) {
  ...
  if (params->anchored) start |= kStartAnchored;
...
bool DFA::Search(const StringPiece& text, const StringPiece& context,
  ...
  *failed = false;
  ...
  if (params.start == DeadState) return false;
...
bool Prog::SearchDFA(const StringPiece& text, const StringPiece& const_context,
  ...
  *failed = false;
  ...
  if (context.data() == NULL) context = text;
  ...
  if (caret && context.begin() != text.begin()) return false;
  if (dollar && context.end() != text.end()) return false;
...
int DFA::BuildAllStates(const Prog::DFAStateCallback& cb) {
  ...
  for (int c = 0; c < 256; c++) {
    ...
    while (c < 256 - 1 && prog_->bytemap()[c + 1] == b) c++;
    input[b] = c;
  }
  ...
  while (!q.empty()) {
    ...
    if (cb) cb(oom ? NULL : output.data(), s == FullMatchState || s->IsMatch());
    if (oom) break;
  }
...
bool DFA::PossibleMatchRange(std::string* min, std::string* max, int maxlen) {
  ...
  if (!AnalyzeSearch(&params)) return false;
  ...
  for (int i = 0; i < maxlen; i++) {
    if (previously_visited_states[s] > kMaxEltRepetitions) break;
    ...
    if (ns == NULL)  // DFA out of memory
      return false;
    if (ns != DeadState && (ns == FullMatchState || ns->IsMatch())) break;
