...
Regexp::~Regexp() {
  if (nsub_ > 0) LOG(DFATAL) << "Regexp not destroyed.";
...
void Regexp::Destroy() {
  ...
  while (stack != NULL) {
    Regexp* re = stack;
    stack = re->down_;
    if (re->ref_ != 0) LOG(DFATAL) << "Bad reference count " << re->ref_;
    if (re->nsub_ > 0) {
      Regexp** subs = re->sub();
...
void Regexp::AddRuneToString(Rune r) {
  ...
  if (nrunes_ == 0) {
    ...
    for (int i = 0; i < nrunes_; i++) runes_[i] = old[i];
    delete[] old;
  }
...
Regexp* Regexp::StarPlusOrQuest(RegexpOp op, Regexp* sub, ParseFlags flags) {
  ...
  if ((sub->op() == kRegexpStar || sub->op() == kRegexpPlus ||
    ...
    if (sub->op() == kRegexpStar) return sub;
...
Regexp* Regexp::ConcatOrAlternate(RegexpOp op, Regexp** sub, int nsub,
  ...
  re->AllocSub(nsub);
  Regexp** subs = re->sub();
  for (int i = 0; i < nsub; i++) subs[i] = sub[i];
  return re;
}
...
Regexp* Regexp::LiteralString(Rune* runes, int nrunes, ParseFlags flags) {
  if (nrunes <= 0) return new Regexp(kRegexpEmptyMatch, flags);
  if (nrunes == 1) return NewLiteral(runes[0], flags);
  Regexp* re = new Regexp(kRegexpLiteralString, flags);
  for (int i = 0; i < nrunes; i++) re->AddRuneToString(runes[i]);
  return re;
}
    ...
    switch (a->op()) {
      ...
      case kRegexpConcat:
        for (int i = 0; i < a->nsub(); i++) {
          a2 = a->sub()[i];
          b2 = b->sub()[i];
          if (!TopEqual(a2, b2)) return false;
          stk.push_back(a2);
          stk.push_back(b2);
        }
        break;
...
std::string RegexpStatus::CodeText(enum RegexpStatusCode code) {
  if (code < 0 || code >= arraysize(kErrorStrings)) code = kRegexpInternalError;
  return kErrorStrings[code];
}

std::string RegexpStatus::Text() const {
  if (error_arg_.empty()) return CodeText(code_);
  std::string s;
...
class NumCapturesWalker : public Regexp::Walker<Ignored> {
 public:
  ...
  virtual Ignored PreVisit(Regexp* re, Ignored ignored, bool* stop) {
    if (re->op() == kRegexpCapture) ncapture_++;
    return ignored;
  }
 ...
 public:
  ...
  virtual Ignored PreVisit(Regexp* re, Ignored ignored, bool* stop) {
    if (re->op() == kRegexpCapture && re->name() != NULL) {
      ...
      if (map_ == NULL) map_ = new std::map<std::string, int>;
...
class CaptureNamesWalker : public Regexp::Walker<Ignored> {
 public:
  ...
  virtual Ignored PreVisit(Regexp* re, Ignored ignored, bool* stop) {
    if (re->op() == kRegexpCapture && re->name() != NULL) {
      // Allocate map once we find a name.
      if (map_ == NULL) map_ = new std::map<int, std::string>;
...
void ConvertRunesToBytes(bool latin1, Rune* runes, int nrunes,
  ...
  if (latin1) {
    bytes->resize(nrunes);
    for (int i = 0; i < nrunes; i++) (*bytes)[i] = static_cast<char>(runes[i]);
  } else {
    bytes->resize(nrunes * UTFmax);  // worst case
    char* p = &(*bytes)[0];
    for (int i = 0; i < nrunes; i++) p += runetochar(p, &runes[i]);
...
bool Regexp::RequiredPrefix(std::string* prefix, bool* foldcase,
  ...
  *suffix = NULL;
  ...
  // 3. the rest
  if (op_ != kRegexpConcat) return false;
  int i = 0;
  while (i < nsub_ && sub()[i]->op_ == kRegexpBeginText) i++;
  if (i == 0 || i >= nsub_) return false;
  Regexp* re = sub()[i];
  if (re->op_ != kRegexpLiteral && re->op_ != kRegexpLiteralString)
    return false;
  i++;
  if (i < nsub_) {
    for (int j = i; j < nsub_; j++) sub()[j]->Incref();
...
bool Regexp::RequiredPrefixForAccel(std::string* prefix, bool* foldcase) {
  ...
  *foldcase = false;
  ...
  while (re->op_ == kRegexpCapture) {
    re = re->sub()[0];
    if (re->op_ == kRegexpConcat && re->nsub_ > 0) re = re->sub()[0];
  }
  if (re->op_ != kRegexpLiteral && re->op_ != kRegexpLiteralString)
    return false;
...
void CharClassBuilder::Negate() {
  ...
  if (it == end()) {
    ...
    if (nextlo <= Runemax) v.push_back(RuneRange(nextlo, Runemax));
  }

  ranges_.clear();
  for (size_t i = 0; i < v.size(); i++) ranges_.insert(v[i]);
...
CharClass* CharClass::Negate() {
  ...
  if (nextlo <= Runemax) cc->ranges_[n++] = RuneRange(nextlo, Runemax);
  cc->nranges_ = n;
  return cc;
}
