...
static bool Satisfy(uint32_t cond, const StringPiece& context, const char* p) {
  ...
  if (cond & kEmptyAllFlags & ~satisfied) return false;
  return true;
}
...
bool Prog::SearchOnePass(const StringPiece& text,
  ...
  for (int i = 0; i < ncap; i++) cap[i] = NULL;

  const char* matchcap[kMaxCap];
  for (int i = 0; i < ncap; i++) matchcap[i] = NULL;

  StringPiece context = const_context;
  if (context.data() == NULL) context = text;
  if (anchor_start() && context.begin() != text.begin()) return false;
  if (anchor_end() && context.end() != text.end()) return false;
  if (anchor_end()) kind = kFullMatch;
  ...
  for (p = bp; p < ep; p++) {
    ...
    if (kind == kFullMatch) goto skipmatch;

    // Not if it's impossible.
    if (matchcond == kImpossible) goto skipmatch;
    ...
    if ((matchcond & kEmptyAllFlags) == 0 || Satisfy(matchcond, context, p)) {
      for (int i = 2; i < 2 * nmatch; i++) matchcap[i] = cap[i];
      ...
      if (kind == kFirstMatch && (cond & kMatchWins)) goto done;
    }

  skipmatch:
    if (state == NULL) goto done;
    if ((cond & kCapMask) && nmatch > 1) ApplyCaptures(cond, p, cap, ncap);
  }
  ...
  // Look for match at end of input.
  {
    ...
    if (matchcond != kImpossible &&
      ...
      for (int i = 2; i < ncap; i++) matchcap[i] = cap[i];
      matchcap[1] = p;
      matched = true;
    }
  }
...
bool Prog::IsOnePass() {
  if (did_onepass_) return onepass_nodes_.data() != NULL;
  did_onepass_ = true;
  ...
  int statesize = sizeof(OneState) + bytemap_range() * sizeof(uint32_t);
  if (maxnodes >= 65000 || dfa_mem_ / 4 / statesize < maxnodes) return false;
  ...
  for (Instq::iterator it = tovisit.begin(); it != tovisit.end(); ++it) {
    ...
    // Default is none.
    for (int b = 0; b < bytemap_range_; b++) node->action[b] = kImpossible;
    ...
    Loop:
      ...
      switch (ip->opcode()) {
        ...
        case kInstNop:
          ...
          if (ip->opcode() == kInstEmptyWidth) cond |= ip->empty();
  ...
  if (ExtraDebug) {  // For debugging, dump one-pass NFA to LOG(ERROR).
    ...
    for (int i = 0; i < size; i++)
      if (nodebyid[i] != -1) idmap[nodebyid[i]] = i;
    ...
    for (Instq::iterator it = tovisit.begin(); it != tovisit.end(); ++it) {
      ...
      for (int i = 0; i < bytemap_range_; i++) {
        if ((node->action[i] & kImpossible) == kImpossible) continue;
