...
NFA::~NFA() {
  delete[] match_;
  for (const Thread& t : arena_) delete[] t.capture;
}
  ...
  Loop:
    ...
    switch (ip->opcode()) {
      ...
      case kInstEmptyWidth:
        ...
        if (ip->empty() & ~Prog::EmptyFlags(context, p)) break;
        a = {ip->out(), NULL};
        goto Loop;
    }
  }
    ...
    switch (ip->opcode()) {
      ...
      case kInstAltMatch:
        ...
        if (ip->greedy(prog_) || longest_) {
          ...
          Decref(t);
          for (++i; i != runq->end(); ++i) {
            if (i->value() != NULL) Decref(i->value());
          }
          runq->clear();
          if (ip->greedy(prog_)) return ip->out1();
          return ip->out();
        }
        break;
...
bool NFA::Search(const StringPiece& text, const StringPiece& const_context,
  ...
  if (prog_->anchor_start() && context.begin() != text.begin()) return false;
  if (prog_->anchor_end() && context.end() != text.end()) return false;
  ...
  for (const char* p = text.data();; p++) {
    ...
    if (id != 0) {
      ...
      for (;;) {
        ...
        switch (ip->opcode()) {
          ...
          case kInstCapture:
            if (ip->cap() < ncapture_) match_[ip->cap()] = p;
            id = ip->out();
            continue;
    ...
    if (runq->size() == 0) {
      if (ExtraDebug) fprintf(stderr, "dead\n");
      break;
    }
  ...
  for (SparseArray<int>::iterator i = fanout->begin(); i != fanout->end();
    ...
    for (SparseSet::iterator j = reachable.begin(); j != reachable.end(); ++j) {
      ...
      switch (ip->opcode()) {
        ...
        case kInstByteRange:
          if (!ip->last()) reachable.insert(id + 1);
