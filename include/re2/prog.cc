...
uint32_t Prog::EmptyFlags(const StringPiece& text, const char* p) {
  ...
  if (p == text.data() && p == text.data() + text.size()) {
    ...
    if (IsWordChar(p[0])) flags |= kEmptyWordBoundary;
  } else if (p == text.data() + text.size()) {
    if (IsWordChar(p[-1])) flags |= kEmptyWordBoundary;
  } else {
    if (IsWordChar(p[-1]) != IsWordChar(p[0])) flags |= kEmptyWordBoundary;
  }
  if (!(flags & kEmptyWordBoundary)) flags |= kEmptyNonWordBoundary;
...
int ByteMapBuilder::Recolor(int oldcolor) {
  ...
  if (it != colormap_.end()) return it->second;
...
void Prog::ComputeByteMap() {
  ...
  for (int id = 0; id < size(); id++) {
    ...
    if (ip->opcode() == kInstByteRange) {
      ...
      if (ip->foldcase() && lo <= 'z' && hi >= 'a') {
        int foldlo = lo;
        int foldhi = hi;
        if (foldlo < 'a') foldlo = 'a';
        if (foldhi > 'z') foldhi = 'z';
        if (foldlo <= foldhi) {
          foldlo += 'A' - 'a';
          foldhi += 'A' - 'a';
  ...
  if (0) {  // For debugging, use trivial bytemap.
    ...
    for (int i = 0; i < 256; i++) bytemap_[i] = static_cast<uint8_t>(i);
    bytemap_range_ = 256;
  }
}
...
void Prog::Flatten() {
  ...
  for (int i = 0; i < kNumInst; i++) inst_count_[i] = 0;
  ...
  int total = 0;
  for (int i = 0; i < kNumInst; i++) total += inst_count_[i];
  ...
  if (size_ <= 512) {
    ...
    for (int i = 0; i < list_count_; ++i) list_heads_[flatmap[i]] = i;
  }
}
...
void Prog::MarkSuccessors(SparseArray<int>* rootmap, SparseArray<int>* predmap,
  ...
  if (!rootmap->has_index(start())) rootmap->set_new(start(), rootmap->size());
    ...
    if (predmap->has_index(id)) {
      for (int pred : (*predvec)[predmap->get_existing(id)]) {
        if (!reachable->contains(pred)) {
          ...
          if (!rootmap->has_index(id)) rootmap->set_new(id, rootmap->size());
        }
      }
    }
  }
...
const void* Prog::PrefixAccel_FrontAndBack(const void* data, size_t size) {
  ...
  for (const char* p = p0;; p++) {
    ...
    if (p == NULL || p[prefix_size_ - 1] == prefix_back_) return p;
  }
}
