...
Regexp::ParseState::~ParseState() {
  ...
  for (Regexp* re = stacktop_; re != NULL; re = next) {
    next = re->down_;
    re->down_ = NULL;
    if (re->op() == kLeftParen) delete re->name_;
    re->Decref();
  }
}
...
bool Regexp::ParseState::PushRegexp(Regexp* re) {
  ...
  if (!IsMarker(re->op())) re->simple_ = re->ComputeSimple();
  re->down_ = stacktop_;
  stacktop_ = re;
  return true;
}
...
const CaseFold* LookupCaseFold(const CaseFold* f, int n, Rune r) {
  ...
  while (n > 0) {
    int m = n / 2;
    if (f[m].lo <= r && r <= f[m].hi) return &f[m];
    if (r < f[m].lo) {
      n = m;
    } else {
      f += m + 1;
...
bool Regexp::ParseState::PushLiteral(Rune r) {
  ...
  if (MaybeConcatString(r, flags_)) return true;
...
bool Regexp::ParseState::PushWordBoundary(bool word) {
  if (word) return PushSimpleOp(kRegexpWordBoundary);
...
bool Regexp::ParseState::DoLeftParen(const StringPiece& name) {
  ...
  re->cap_ = ++ncap_;
  if (name.data() != NULL) re->name_ = new std::string(name);
  return PushRegexp(re);
}
...
Regexp* Regexp::LeadingRegexp(Regexp* re) {
  if (re->op() == kRegexpEmptyMatch) return NULL;
  if (re->op() == kRegexpConcat && re->nsub() >= 2) {
    Regexp** sub = re->sub();
    if (sub[0]->op() == kRegexpEmptyMatch) return NULL;
    return sub[0];
  }
  return re;
}
...
Regexp* Regexp::RemoveLeadingRegexp(Regexp* re) {
  if (re->op() == kRegexpEmptyMatch) return re;
  if (re->op() == kRegexpConcat && re->nsub() >= 2) {
    Regexp** sub = re->sub();
    if (sub[0]->op() == kRegexpEmptyMatch) return re;
    sub[0]->Decref();
    sub[0] = NULL;
    if (re->nsub() == 2) {
...
Rune* Regexp::LeadingString(Regexp* re, int* nrune, Regexp::ParseFlags* flags) {
  while (re->op() == kRegexpConcat && re->nsub() > 0) re = re->sub()[0];
  ...
  for (;;) {
    ...
    if (splices.empty()) {
      ...
      for (int i = 0; i < nsub;) {
        ...
        while (sub + i < iter->sub) sub[out++] = sub[i++];
        switch (round) {
          case 1:
          case 2: {
...
void Regexp::ParseState::DoCollapse(RegexpOp op) {
  ...
  if (stacktop_ != NULL && stacktop_->down_ == next) return;
...
bool Regexp::ParseState::MaybeConcatString(int r, ParseFlags flags) {
  Regexp* re1;
  Regexp* re2;
  if ((re1 = stacktop_) == NULL || (re2 = re1->down_) == NULL) return false;
  ...
  if (re1->op_ == kRegexpLiteral) {
    ...
    for (int i = 0; i < re1->nrunes_; i++) re2->AddRuneToString(re1->runes_[i]);
    re1->nrunes_ = 0;
    delete[] re1->runes_;
    re1->runes_ = NULL;
  }
...
static bool ParseInteger(StringPiece* s, int* np) {
  if (s->empty() || !isdigit((*s)[0] & 0xFF)) return false;
  // Disallow leading zeros.
  if (s->size() >= 2 && (*s)[0] == '0' && isdigit((*s)[1] & 0xFF)) return false;
  int n = 0;
  int c;
...
static bool MaybeParseRepetition(StringPiece* sp, int* lo, int* hi) {
  ...
  s.remove_prefix(1);  // '{'
  if (!ParseInteger(&s, lo)) return false;
  ...
  if (s[0] == ',') {
    ...
    if (s[0] == '}') {
      ...
      *hi = -1;
      ...
      // {2,4} means 2, 3, or 4.
      if (!ParseInteger(&s, hi)) return false;
    }
  } else {
    // {2} means exactly two
    *hi = *lo;
...
static bool IsValidUTF8(const StringPiece& s, RegexpStatus* status) {
  ...
  Rune r;
  while (!t.empty()) {
    if (StringPieceToRune(&r, &t, status) < 0) return false;
  }
  return true;
}
...
static bool ParseEscape(StringPiece* s, Rune* rp, RegexpStatus* status,
  ...
  if (StringPieceToRune(&c, s, status) < 0) return false;
  int code;
  switch (c) {
    default:
    ...
    case '7':
      ...
      if (s->empty() || (*s)[0] < '0' || (*s)[0] > '7') goto BadEscape;
      FALLTHROUGH_INTENDED;
    case '0':
      ...
      if (code > rune_max) goto BadEscape;
      *rp = code;
      return true;

    // Hexadecimal escapes
    case 'x':
      if (s->empty()) goto BadEscape;
      if (StringPieceToRune(&c, s, status) < 0) return false;
      if (c == '{') {
        ...
        if (c != '}' || nhex == 0) goto BadEscape;
        *rp = code;
        return true;
      }
      // Easy case: two hex digits.
      if (s->empty()) goto BadEscape;
      if (StringPieceToRune(&c1, s, status) < 0) return false;
      if (!IsHex(c) || !IsHex(c1)) goto BadEscape;
...
static const UGroup* LookupGroup(const StringPiece& name, const UGroup* groups,
  ...
  for (int i = 0; i < ngroups; i++)
    if (StringPiece(groups[i].name) == name) return &groups[i];
  return NULL;
}
...
static const UGroup* LookupUnicodeGroup(const StringPiece& name) {
  ...
  if (name == StringPiece("Any")) return &anygroup;
...
static void AddUGroup(CharClassBuilder* cc, const UGroup* g, int sign,
  ...
  if (sign == +1) {
    ...
    if (next <= Runemax) cc->AddRangeFlags(next, Runemax, parse_flags);
  }
}
...
const UGroup* MaybeParsePerlCCEscape(StringPiece* s,
                                     Regexp::ParseFlags parse_flags) {
  if (!(parse_flags & Regexp::PerlClasses)) return NULL;
  if (s->size() < 2 || (*s)[0] != '\\') return NULL;
...
ParseStatus ParseUnicodeGroup(StringPiece* s, Regexp::ParseFlags parse_flags,
  ...
  if (!(parse_flags & Regexp::UnicodeGroups)) return kParseNothing;
  if (s->size() < 2 || (*s)[0] != '\\') return kParseNothing;
  Rune c = (*s)[1];
  if (c != 'p' && c != 'P') return kParseNothing;
  ...
  if (!StringPieceToRune(&c, s, status)) return kParseError;
  if (c != '{') {
    ...
    if (end == StringPiece::npos) {
      if (!IsValidUTF8(seq, status)) return kParseError;
    ...
    if (!IsValidUTF8(name, status)) return kParseError;
  }
...
static ParseStatus ParseCCName(StringPiece* s, Regexp::ParseFlags parse_flags,
  ...
  if (ep - p < 2 || p[0] != '[' || p[1] != ':') return kParseNothing;
  ...
  if (q > ep - 2) return kParseNothing;
...
bool Regexp::ParseState::ParseCCCharacter(StringPiece* s, Rune* rp,
  ...
  if ((*s)[0] == '\\') return ParseEscape(s, rp, status, rune_max_);
...
bool Regexp::ParseState::ParseCCRange(StringPiece* s, RuneRange* rr,
                                      ...
                                      RegexpStatus* status) {
  StringPiece os = *s;
  if (!ParseCCCharacter(s, &rr->lo, whole_class, status)) return false;
  // [a-] means (a|-), so check for final ].
  if (s->size() >= 2 && (*s)[0] == '-' && (*s)[1] != ']') {
    s->remove_prefix(1);  // '-'
    if (!ParseCCCharacter(s, &rr->hi, whole_class, status)) return false;
    if (rr->hi < rr->lo) {
      status->set_code(kRegexpBadCharRange);
      status->set_error_arg(
...
bool Regexp::ParseState::ParsePerlFlags(StringPiece* s) {
  ...
  if (t.size() > 2 && t[0] == 'P' && t[1] == '<') {
    ...
    size_t end = t.find('>', 2);
    if (end == StringPiece::npos) {
      if (!IsValidUTF8(*s, status_)) return false;
    ...
    if (!IsValidUTF8(name, status_)) return false;
  ...
  for (bool done = false; !done;) {
    if (t.empty()) goto BadPerlOp;
    if (StringPieceToRune(&c, &t, status_) < 0) return false;
    switch (c) {
      default:
        goto BadPerlOp;
  ...
  if (negated && !sawflags) goto BadPerlOp;
...
Regexp* Regexp::Parse(const StringPiece& s, ParseFlags global_flags,
  ...
  if (global_flags & Literal) {
    ...
    while (!t.empty()) {
      Rune r;
      if (StringPieceToRune(&r, &t, status) < 0) return NULL;
      if (!ps.PushLiteral(r)) return NULL;
    }
    return ps.DoFinish();
  }
    ...
    switch (t[0]) {
      ...
      case '(':
        ...
        if ((ps.flags() & PerlX) && (t.size() >= 2 && t[1] == '?')) {
          ...
          if (!ps.ParsePerlFlags(&t)) return NULL;
          break;
        }
        if (ps.flags() & NeverCapture) {
          if (!ps.DoLeftParenNoCapture()) return NULL;
        } else {
          if (!ps.DoLeftParen(StringPiece())) return NULL;
        }
        t.remove_prefix(1);  // '('
        break;

      case '|':
        if (!ps.DoVerticalBar()) return NULL;
        t.remove_prefix(1);  // '|'
        break;

      case ')':
        if (!ps.DoRightParen()) return NULL;
        t.remove_prefix(1);  // ')'
        break;

      case '^':  // Beginning of line.
        if (!ps.PushCaret()) return NULL;
        t.remove_prefix(1);  // '^'
        break;

      case '$':  // End of line.
        if (!ps.PushDollar()) return NULL;
        t.remove_prefix(1);  // '$'
        break;

      case '.':  // Any character (possibly except newline).
        if (!ps.PushDot()) return NULL;
        t.remove_prefix(1);  // '.'
        break;

      case '[': {  // Character class.
        Regexp* re;
        if (!ps.ParseCharClass(&t, &re, status)) return NULL;
        if (!ps.PushRegexp(re)) return NULL;
        break;
      }
      ...
      case '*': {  // Zero or more.
        ...
        Rep:
          ...
          if (!ps.PushRepeatOp(op, opstr, nongreedy)) return NULL;
          isunary = opstr;
          break;
      }
      ...
      case '{': {  // Counted repetition.
        ...
        opstr = StringPiece(opstr.data(),
                            static_cast<size_t>(t.data() - opstr.data()));
        if (!ps.PushRepetition(lo, hi, opstr, nongreedy)) return NULL;
        isunary = opstr;
        break;
      }
      ...
      case '\\': {  // Escaped character or Perl sequence.
        ...
        if ((ps.flags() & Regexp::PerlB) && t.size() >= 2 &&
            (t[1] == 'b' || t[1] == 'B')) {
          if (!ps.PushWordBoundary(t[1] == 'b')) return NULL;
          t.remove_prefix(2);  // '\\', 'b'
          break;
        }
        ...
        if ((ps.flags() & Regexp::PerlX) && t.size() >= 2) {
          if (t[1] == 'A') {
            if (!ps.PushSimpleOp(kRegexpBeginText)) return NULL;
            t.remove_prefix(2);  // '\\', 'A'
            break;
          }
          if (t[1] == 'z') {
            if (!ps.PushSimpleOp(kRegexpEndText)) return NULL;
            t.remove_prefix(2);  // '\\', 'z'
            break;
          }
          ...
          if (t[1] == 'C') {  // \C: any byte [sic]
            if (!ps.PushSimpleOp(kRegexpAnyByte)) return NULL;
            t.remove_prefix(2);  // '\\', 'C'
            break;
          }
        ...
        Rune r;
        if (!ParseEscape(&t, &r, status, ps.rune_max())) return NULL;
        if (!ps.PushLiteral(r)) return NULL;
        break;
      }
    }
