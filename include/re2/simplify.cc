  ...
  switch (op_) {
    ...
    case kRegexpAlternate:
      ...
      for (int i = 0; i < nsub_; i++)
        if (!subs[i]->simple()) return false;
      return true;
    case kRegexpCharClass:
      // Simple as long as the char class is not empty, not full.
      if (ccb_ != NULL) return !ccb_->empty() && !ccb_->full();
...
Regexp* CoalesceWalker::PostVisit(Regexp* re, Regexp* parent_arg,
                                  ...
                                  int nchild_args) {
  if (re->nsub() == 0) return re->Incref();

  if (re->op() != kRegexpConcat) {
    if (!ChildArgsChanged(re, child_args)) return re->Incref();

    // Something changed. Build a new op.
    Regexp* nre = new Regexp(re->op(), re->parse_flags());
    nre->AllocSub(re->nsub());
    Regexp** nre_subs = nre->sub();
    for (int i = 0; i < re->nsub(); i++) nre_subs[i] = child_args[i];
  ...
  for (int i = n; i < re->nsub(); i++) {
    if (child_args[i]->op() == kRegexpEmptyMatch) n++;
  }
  // Build a new op.
  Regexp* nre = new Regexp(re->op(), re->parse_flags());
...
void CoalesceWalker::DoCoalesce(Regexp** r1ptr, Regexp** r2ptr) {
  ...
  switch (r2->op()) {
    ...
    case kRegexpLiteralString: {
      ...
      int n = 1;
      while (n < r2->nrunes() && r2->runes()[n] == r) n++;
      nre->min_ += n;
      if (nre->max() != -1) nre->max_ += n;
      if (n == r2->nrunes()) goto LeaveEmpty;
      *r1ptr = nre;
...
Regexp* SimplifyWalker::PostVisit(Regexp* re, Regexp* parent_arg,
  ...
  switch (re->op()) {
    ...
    case kRegexpQuest: {
      ...
      if (newsub->op() == kRegexpEmptyMatch) return newsub;
...
Regexp* SimplifyWalker::SimplifyRepeat(Regexp* re, int min, int max,
  ...
  if (max == -1) {
    ...
    if (min == 0) return Regexp::Star(re->Incref(), f);

    // Special case: x{1,} is x+
    if (min == 1) return Regexp::Plus(re->Incref(), f);

    // General case: x{4,} is xxxx+
    PODArray<Regexp*> nre_subs(min);
    for (int i = 0; i < min - 1; i++) nre_subs[i] = re->Incref();
    nre_subs[min - 1] = Regexp::Plus(re->Incref(), f);
    return Regexp::Concat(nre_subs.data(), min, f);
  }

  // Special case: (x){0} matches only empty string.
  if (min == 0 && max == 0) return new Regexp(kRegexpEmptyMatch, f);

  // Special case: x{1} is just x.
  if (min == 1 && max == 1) return re->Incref();
  ...
  if (min > 0) {
    PODArray<Regexp*> nre_subs(min);
    for (int i = 0; i < min; i++) nre_subs[i] = re->Incref();
...
Regexp* SimplifyWalker::SimplifyCharClass(Regexp* re) {
  ...
  // Special cases
  if (cc->empty()) return new Regexp(kRegexpNoMatch, re->parse_flags());
  if (cc->full()) return new Regexp(kRegexpAnyChar, re->parse_flags());
