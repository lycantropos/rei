...
static std::string trunc(const StringPiece& pattern) {
  if (pattern.size() < 100) return std::string(pattern);
...
int RE2::Options::ParseFlags() const {
  ...
  switch (encoding()) {
    default:
      if (log_errors()) LOG(ERROR) << "Unknown encoding " << encoding();
      break;
  ...
  if (!posix_syntax()) flags |= Regexp::LikePerl;

  if (literal()) flags |= Regexp::Literal;

  if (never_nl()) flags |= Regexp::NeverNL;

  if (dot_nl()) flags |= Regexp::DotNL;

  if (never_capture()) flags |= Regexp::NeverCapture;

  if (!case_sensitive()) flags |= Regexp::FoldCase;

  if (perl_classes()) flags |= Regexp::PerlClasses;

  if (word_boundary()) flags |= Regexp::PerlB;

  if (one_line()) flags |= Regexp::OneLine;
...
RE2::~RE2() {
  if (suffix_regexp_) suffix_regexp_->Decref();
  if (entire_regexp_) entire_regexp_->Decref();
  delete prog_;
  delete rprog_;
...
static int Fanout(Prog* prog, std::vector<int>* histogram) {
  ...
  if (histogram != NULL) histogram->assign(data, data + size);
  return size - 1;
}
...
const std::map<std::string, int>& RE2::NamedCapturingGroups() const {
  std::call_once(
      ...
      [](const RE2* re) {
        ...
        if (re->named_groups_ == NULL) re->named_groups_ = empty_named_groups;
      },
      this);
...
const std::map<int, std::string>& RE2::CapturingGroupNames() const {
  std::call_once(
      ...
      [](const RE2* re) {
        ...
        if (re->group_names_ == NULL) re->group_names_ = empty_group_names;
      },
      this);
  return *group_names_;
}
...
bool RE2::Replace(std::string* str, const RE2& re, const StringPiece& rewrite) {
  ...
  if (nvec > 1 + re.NumberOfCapturingGroups()) return false;
  if (nvec > static_cast<int>(arraysize(vec))) return false;
  if (!re.Match(*str, 0, str->size(), UNANCHORED, vec, nvec)) return false;

  std::string s;
  if (!re.Rewrite(&s, rewrite, vec, nvec)) return false;
...
int RE2::GlobalReplace(std::string* str, const RE2& re,
  ...
  while (p <= ep) {
    ...
    if (!re.Match(*str, static_cast<size_t>(p - str->data()), str->size(),
                  UNANCHORED, vec, nvec))
      break;
    if (p < vec[0].data()) out.append(p, vec[0].data() - p);
...
bool RE2::Extract(const StringPiece& text, const RE2& re,
  ...
  if (nvec > 1 + re.NumberOfCapturingGroups()) return false;
  if (nvec > static_cast<int>(arraysize(vec))) return false;
  if (!re.Match(text, 0, text.size(), UNANCHORED, vec, nvec)) return false;
...
bool RE2::Match(const StringPiece& text, size_t startpos, size_t endpos,
  ...
  if (!ok()) {
    if (options_.log_errors()) LOG(ERROR) << "Invalid RE2: " << *error_;
    return false;
  }
  ...
  if (nsubmatch == 0) matchp = NULL;

  int ncap = 1 + NumberOfCapturingGroups();
  if (ncap > nsubmatch) ncap = nsubmatch;

  // If the regexp is anchored explicitly, must not be in middle of text.
  if (prog_->anchor_start() && startpos != 0) return false;
  if (prog_->anchor_end() && endpos != text.size()) return false;
  ...
  switch (re_anchor) {
    ...
    case UNANCHORED: {
      ...
      if (!prog->SearchDFA(match, text, Prog::kAnchored, Prog::kLongestMatch,
        ...
        if (options_.log_errors()) LOG(ERROR) << "SearchDFA inconsistency";
        return false;
      }
      break;
    }
    ...
    case ANCHOR_BOTH:
    case ANCHOR_START:
      if (re_anchor == ANCHOR_BOTH) kind = Prog::kFullMatch;
  ...
  for (int i = ncap; i < nsubmatch; i++) submatch[i] = StringPiece();
  return true;
}
...
bool RE2::Rewrite(std::string* out, const StringPiece& rewrite,
  ...
  for (const char *s = rewrite.data(), *end = s + rewrite.size(); s < end;
    ...
    if (isdigit(c)) {
      ...
      if (!snip.empty()) out->append(snip.data(), snip.size());
    } else if (c == '\\') {
      out->push_back('\\');
    } else {
      if (options_.log_errors())
