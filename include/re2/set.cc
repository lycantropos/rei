...
RE2::Set::Set(const RE2::Options& options, RE2::Anchor anchor)
    : options_(options), anchor_(anchor), compiled_(false), size_(0) {
...
int RE2::Set::Add(const StringPiece& pattern, std::string* error) {
  ...
  if (re == NULL) {
    if (error != NULL) *error = status.Text();
  ...
  if (re->op() == kRegexpConcat) {
    ...
    for (int i = 0; i < nsub; i++) sub[i] = re->sub()[i]->Incref();
    sub[nsub] = m;
    re->Decref();
...
bool RE2::Set::Compile() {
  ...
  for (int i = 0; i < size_; i++) sub[i] = elem_[i].second;
  elem_.clear();
  elem_.shrink_to_fit();
...
bool RE2::Set::Match(const StringPiece& text, std::vector<int>* v,
  ...
  if (!compiled_) {
    ...
    if (error_info != NULL) error_info->kind = kNotCompiled;
    return false;
  }
  ...
  if (dfa_failed) {
    ...
    if (error_info != NULL) error_info->kind = kOutOfMemory;
    return false;
  }
  if (ret == false) {
    if (error_info != NULL) error_info->kind = kNoError;
    return false;
  }
  if (v != NULL) {
    if (matches->empty()) {
      LOG(DFATAL) << "RE2::Set::Match() matched, but no matches returned?!";
      if (error_info != NULL) error_info->kind = kInconsistent;
      return false;
    }
